...
``{parent}`` and ``{n}``). Renames happen in place, and are journalled so they
can be undone.
"""
...
from .rename_profiles import Profile, apply_rules, render_pattern
...
def generic_new_name(profile: Profile, stem: str, parent: str, n: int) -> str:
    """Return the profile-formatted name for *stem* (without extension)."""
    cleaned = apply_rules(stem, profile.rules) or stem
    rendered = render_pattern(
        profile.pattern("generic"),
        {"name": cleaned, "parent": parent, "n": n},
    )
...
def plan_generic_rename(
    ...
    def parent_label(directory: Path) -> str:
        return apply_rules(directory.name, profile.rules) or directory.name
    ...
    def plan_level(directory: Path, depth: int) -> None:
        ...
        def plan_entry(entry: Path, n: int, kind: str) -> None:
            ...
            new_stem = generic_new_name(profile, stem, parent, n)
...
def preview_generic_name(profile: Profile, name: str, parent: str = "Parent", n: int = 1) -> str:
    """Format a single sample name (used by the profile tester and AI verification)."""
    return generic_new_name(profile, name, apply_rules(parent, profile.rules) or parent, n)
